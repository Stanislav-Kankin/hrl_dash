...
async function checkAuthAndInitialize() {
    ...
    if (!token) {
        console.log('❌ No token found, showing auth modal immediately');
        showAuthModal();
        authChecked = true;
        return;
    }
    ...
    try {
        ...
        if (error.message.includes('Authentication required') || error.message.includes('401')) {
            console.log('🔐 401 error, clearing token and showing modal');
...
async function initializeDashboard() {
    try {
        ...
        if (!connection.connected) {
            console.warn('⚠️ Внимание: подключение к Bitrix24 не настроено');
        }
...
async function login(event) {
    ...
    try {
        ...
        if (data.access_token) {
            ...
            hideAuthModal();
            // Сбрасываем флаг проверки авторизации для повторной проверки
            authChecked = false;
            await checkAuthAndInitialize();
...
function updateUIForAuth() {
    ...
    if (header && currentUser) {
        ...
        console.log('✅ User info added to header');
    }
}
...
// Переопределяем функции из main.html
